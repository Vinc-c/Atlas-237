...
-- Root cause: seed_demo_data() inserted gen_random_uuid() into
-- approvals.ai_task_id which has a FK to ai_tasks(id) — the random
-- UUID never matches a real ai_tasks row, so the FK constraint
-- fails, which rolls back the entire handle_new_user() transaction,
-- which makes Supabase auth abort user creation with
-- "Database error saving new user".
--
-- Fix 1: Set approvals.ai_task_id to NULL (it is nullable) in seed.
-- Fix 2: Make handle_new_user resilient — wrap the seed call in an
--         exception block so that even if seeding fails for any
--         reason, the user/org/profile creation still succeeds.
-- Fix 3: Add a subscriptions table for Flutterwave billing + trial
--         enforcement paywall.
-- ============================================================

-- ============================================================
-- FIX 1: Rewrite seed_demo_data with corrected approvals inserts
-- ============================================================
...
BEGIN
  ...
  -- AI Tasks (capture real IDs for approvals FK)
  ...
  -- Approvals — use REAL ai_task_id (was gen_random_uuid → FK violation)
  ...
  VALUES
    (new_org_id, 'Production API', 'atlas_prod_', true, owner_id);

  -- Webhooks
  INSERT INTO webhooks (org_id, url, events, active)
  VALUES
    (new_org_id, 'https://example.com/webhooks/atlas', ARRAY['contact.created', 'deal.won', 'invoice.paid'], true);

END;
$$;

-- ============================================================
-- FIX 2: Resilient handle_new_user — seed failure must NOT
--         block user/org/profile creation
-- ============================================================
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- log and continue — user creation must not fail
...
-- Re-attach the trigger (drop + recreate to pick up the new function)
...
-- ============================================================
-- FIX 3: Subscriptions table for Flutterwave billing + paywall
-- ============================================================
...
-- Helper: check if org has active subscription or is in trial

-- 027: seed_demo_data was marking Gmail and Stripe as "connected" for every
-- brand-new signup, alongside an "active" Production API key with no real
-- secret and an "active" webhook pointed at https://example.com. None of
-- that was ever really connected — a new customer would open Integrations
-- and see Gmail/Stripe already green/"Connected" despite never having
-- authorized anything, and Settings > API Keys would show a live-looking
-- production key that was never actually issued.
--
-- This migration keeps the rest of the sample data (example deals, contacts,
-- tickets, campaigns, workflow definitions — ordinary onboarding content to
-- help a new user explore the UI) and only removes the three inserts that
-- falsely claimed a real external connection or a real live credential.
...
BEGIN
  ...
  -- NOTE: previously also inserted fake "connected" rows here for gmail and
  -- stripe. Removed — a brand-new org has connected nothing yet, and
  -- claiming otherwise misleads the user and can mask real integration
  -- bugs (an agent "using" a fake-connected Gmail would fail unexpectedly).
  ...
  -- NOTE: previously also inserted a fake "active" Production API key here
  -- (no real secret was ever generated for it — key_prefix alone is not a
  -- usable credential) and a fake "active" webhook pointed at
  -- https://example.com/webhooks/atlas. Both removed for the same reason:
  -- an org that just signed up has issued no real API key and configured
  -- no real webhook, and showing one as already active is misleading.
END;
$$;

...
export function MarketplacePage() {
  ...
  async function confirmConnect() {
    if (!configApp) return;
    setConnecting(true);
    const fullKey = 'atlas_' + configApp.provider + '_' + Math.random().toString(36).substring(2, 18);
    ...
    const { error } = await supabase.from('integrations').insert({
      ...
      config: { ...configData, key_prefix: keyPrefix },
    });
    if (!error) {
          ...
          {configApp?.authType === 'oauth' ? (
            <div className="rounded-lg bg-primary-50 border border-primary-200 p-4 text-sm text-primary-800">
              ...
              <p className="text-primary-700 mb-3">
                ...
                {lang === 'fr' ? ' pour autoriser l\'accès.' : ' to authorize access.'}
              </p>
              <button onClick={confirmConnect} disabled={connecting} className="btn-primary btn-sm w-full">
...
export function APIWebhooksPage() {
  ...
  async function createKey() {
    ...
    const fullKey = 'atlas_sk_' + Math.random().toString(36).substring(2, 10) + Math.random().toString(36).substring(2, 18);
  ...
  async function createWebhook() {
    ...
    const secret = 'whsec_' + Math.random().toString(36).substring(2, 18) + Math.random().toString(36).substring(2, 18);
  ...
  async function testWebhook(wh: WebhookType) {
    ...
    try {
      const res = await fetch(wh.url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Atlas-Signature': wh.secret || '' },
        body: JSON.stringify({ event: 'test.ping', timestamp: new Date().toISOString(), data: { test: true } }),
      });

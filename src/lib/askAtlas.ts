...
/**
 * Tries the real AI assistant (whichever provider the org configured —
 * platform-wide free Gemini by default, or the org's own OpenAI/Anthropic/
 * Gemini key once selected in Settings) first. Returns null ONLY when the
 * edge function couldn't be reached at all (no session, network failure,
 * unparseable response) — in that case the caller falls back to the
 * deterministic keyword-matched answers below, so the feature stays usable
 * offline/misconfigured rather than breaking outright.
 *
 * Critically, this does NOT return null when the edge function WAS reached
 * and responded with a real failure reason (bad/missing API key, the
 * provider itself rejecting the key, etc.) — that case returns the actual
 * error message instead. Silently falling back to the canned keyword bot
 * on every real AI failure was the bug here: a user (or org admin) testing
 * their own connected key would get what looks like a normal, working
 * answer with zero indication it was never actually generated by their
 * chosen AI at all, making a real misconfiguration undiagnosable from the
 * UI. Prefixed with an explicit warning so it's never mistaken for either
 * a real AI answer or a keyword-matched one.
 */
async function tryRealAI(question: string, lang: string): Promise<Answer | null> {
  try {
    ...
    if (res.ok && result?.ok && result.text) {
      return { text: result.text };
    }
    if (result?.msg) {
      console.error('[askAtlas] AI assistant returned an error:', result.msg);
      return { text: lang === 'fr' ? `⚠️ IA indisponible : ${result.msg}` : `⚠️ AI unavailable: ${result.msg}` };
    }
    return null;
  } catch (err) {
    console.error('[askAtlas] Could not reach the AI assistant function:', err);
    return null;
  }
}

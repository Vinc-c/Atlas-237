...
import { Users, Building2, UserPlus, Handshake, KanbanSquare, Activity, Calendar, ArrowRightCircle, List as ListIcon } from 'lucide-react';
  ...
  return (
    <>
      <ListPage<Lead>
        ...
        orderBy="created_at"
        rowActions={(row) => row.status !== 'converted' ? (
          <button
            onClick={() => setConvertingLead(row)}
            title="Convert lead"
            className="p-1.5 rounded text-primary-500 hover:bg-primary-50 hover:text-primary-700 transition-colors"
          >
...
export function DealsPage() {
  ...
  const viewToggle = (
    <div className="flex items-center rounded-lg border border-ink-200 p-0.5">
      <button
        ...
        className={`flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${view === 'kanban' ? 'bg-primary-600 text-white' : 'text-ink-500 hover:bg-ink-50'}`}
      >
        <KanbanSquare size={13} /> Kanban
      </button>
      <button
        onClick={() => setView('list')}
        className={`flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${view === 'list' ? 'bg-primary-600 text-white' : 'text-ink-500 hover:bg-ink-50'}`}
      >
  ...
  if (view === 'kanban') {
    return <DealsKanban title={t('nav.deals', language)} headerAction={viewToggle} />;
  }
